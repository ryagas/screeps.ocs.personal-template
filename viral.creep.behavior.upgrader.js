...
mod.approach = function(creep) {
   // console.log("approaching! " + creep.name);
...
mod.run = function(creep) {
    var flag = _.filter(Game.flags, function(f) {
        return f.pos.roomName == creep.pos.roomName && f.color == COLOR_WHITE && f.secondaryColor == COLOR_PURPLE
    });
    if (flag.length > 0) {
        if (creep.memory.gotBoosted) {
            this.runNormalCode(creep);
        } else {
            var lab = flag[0].pos.findInRange(FIND_STRUCTURES, 0, {
                filter: (s) => {
                    return s.structureType == STRUCTURE_LAB && (
                            s.mineralType == RESOURCE_GHODIUM_HYDRIDE ||
                            s.mineralType == RESOURCE_GHODIUM_ACID ||
                            s.mineralType == RESOURCE_CATALYZED_GHODIUM_ACID
                        ) &&
                        s.mineralAmount >= 30
                }
            });
            console.log(lab);
            if (lab.length > 0) {
                var res = lab[0].boostCreep(creep);
...
mod.runNormalCode = function(creep){
